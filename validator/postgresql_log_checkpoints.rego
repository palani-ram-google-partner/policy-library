...
deny[{
	...
	settings.databaseFlags[i].name == "log_checkpoints"
	settings.databaseFlags[i].value != "on"

	message := sprintf("%v Ensure that the log_checkpoints database flag for Cloud SQL PostgreSQL instance is set to ON", [asset.name])

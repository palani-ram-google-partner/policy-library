...
deny[{
	...
	metadata_config.items[i].key == "serial-port-enable"
	metadata_config.items[i].value != "false"

	message := sprintf("%v Enable connecting to serial ports should not be enabled for VM instance.", [asset.name])

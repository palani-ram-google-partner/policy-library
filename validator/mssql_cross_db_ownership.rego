...
deny[{
	...
	key = "cross db ownership chaining"
...
# check for cross db ownership chaining under settings databaseflags - no violation
settings_databaseflags(settings) {
	setdata := settings.databaseFlags[_]
	setdata.name == "cross db ownership chaining"
	setdata.value != "off"
}
